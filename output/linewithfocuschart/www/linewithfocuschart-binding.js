...
binding.find = function(scope) {
  ...
  // this binding.
  return $(scope).find(".nvd3-linewithfocuschart");
};
...
binding.renderValue = function(el, input) {
  ...
  var isDisabled = $el.find('g.nv-legend').find('g.nv-series.disabled')
  if( isDisabled.length < 1 ){
    ...
    for(i=0; i<input.data.length; i++){
      var series = $el.find('g.nv-legend').children().children()[i]
      if( $(series).attr('class').split(" ").indexOf('disabled') < 0){
        input.data[i].disabled = false;
      }else{
        input.data[i].disabled = true;
      }
    }
  }
...
Shiny.outputBindings.register(binding, "shinyjsexamples.nvd3-linewithfocuschart");
