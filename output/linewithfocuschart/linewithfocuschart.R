...
lineWithFocusChartOutput <- function(inputId, width="100%", height="400px") {
  ...
  tagList(
    ...
    singleton(tags$head(
      ...
      tags$script(src="linewithfocuschart-binding.js")
    )),
    div(id=inputId, class="nvd3-linewithfocuschart", style=style,
      tag("svg", list())
    )
  )
}
...
renderLineWithFocusChart <- function(expr, env=parent.frame(), quoted=FALSE) {
