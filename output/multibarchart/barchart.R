...
multiBarChartOutput <- function(inputId, width="100%", height="400px") {
...
renderMultiBarChart <- function(expr, env=parent.frame(), quoted=FALSE) {
  ...
  function() {
    ...
    if(is.null(dataframe)){
      ...
      # Pull x values out of dataframe if there is a column named x or X
      ...
      if(is.factor(xVals)){
        xVals = as.integer(as.character(xVals))
      }
      ...
      if(exists('xlim')){
        ...
        # remove the xlim variable so that the graph does not get messed up
        result$xlim = NULL
      ...
      c(list(
        data = mapply(function(col, name, color) {
          ...
          values <- mapply(function(val, i) {
              list(x = i, y = val)
          }, col, as.character(xVals), SIMPLIFY=FALSE, USE.NAMES=FALSE)
